...
#         (df['yr_renovated'] > 2010)
    ...
    # create a new column and use np.select to assign values to it using our lists as arguments
    df['yr_renovated_bin'] = np.select(conditions, values)
...
def combine_yr_built_renov(df):
    ...
    df_bins['built_renov_bin'] = 'yr built bin: '+ df_bins['yr_built_bin'] + " renovation category:" + df_bins['yr_renovated_bin']
    df_bins.drop(['yr_built_bin', 'yr_renovated_bin'], axis=1, inplace=True)
    return df_bins